...
class Visualize(QWidget):
    invariants_selected = {}
    dic_invariants: dict[str, Invariant]

    def __init__(self, project_window):
        super().__init__()

        # TODO: import should come from the Wizard's FilterList, fix it after the wizard's refactor.
        self.dic_invariants = {**dic_num_invariants_names, **dic_bool_invariants_names} #dic_name_to_calc. Change name_inv.calc
        self.graph = None
        self.info = None
    ...
    def create_tool_bar(self):
        if self.tool_bar is None:
            ...
            self.left_button.setIcon(QtGui.QIcon("view/resources/icons/left_arrow_key.png"))
            self.left_button.setIconSize(QtCore.QSize(20, 20))

            self.left_button.setDisabled(True)

            self.right_button.setIcon(QtGui.QIcon("view/resources/icons/right_arrow_key.png"))
