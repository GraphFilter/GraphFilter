...
from source.worker.export_latex_worker import ExportLatexWorker
...
class Controller:
    ...
    def finish_filter(self, list_graphs):
        export = ExportGraphsWorker(list_graphs, self.wizard_window.parameters.location, self.wizard_window.parameters.name)
        latex = ExportLatexWorker(self.wizard_window.parameters, list_graphs, len(list_graphs))
        latex.generate()
