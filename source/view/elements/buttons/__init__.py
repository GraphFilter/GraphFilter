...
class GenericButton(QPushButton):
    ...
    def set_content_attributes(self):
        ...
        if self.translation_object:
            if self.icon:
                icon = qtawesome.icon(self.icon, color=color)
    ...
    def get_minimum_size(self):
        ...
        return self.fontMetrics().horizontalAdvance(self.translation_object.name) + (
                    self.iconSize().width() * icon_addition)
...
class ListButton:
    ...
    def get_max_button_width(self, widget_object: QWidget):
        return max(self.buttons, key=lambda button: button.get_width(widget_object))
    ...
    def factory(objects: list[TranslationObject],
        ...
        for item in objects:
            ...
            iconable_list.append(button_type(**generic_params))

        return iconable_list
